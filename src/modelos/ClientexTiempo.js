...
ClientexTiempo.getInformeCliente = function (callback) {
    if (connection) {
        var sql = "SELECT r.id_cliente " +
            ",concat(COALESCE(c.primer_nombre,''),' ',COALESCE( c.segundo_nombre,''), ' ',COALESCE(c.primer_apellido,''),' ',COALESCE(c.segundo_apellido,'')) as Nombre_Cliente,"+
            ",e.nombre_elemento,"+
